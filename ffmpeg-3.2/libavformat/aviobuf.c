...
{
    /** 如果 s->buf_ptr 中残留的数据大于size, 将 data 指向这个数据的开始，然后返回这个数据的大小；
     *  否则将调用 avio_read 中读取指定大小的数据到 buf 中
     */
...
int ffio_fdopen(AVIOContext **s, URLContext *h)
{
    ...
    buffer = av_malloc(buffer_size); /** 初始化 buffer */
    if (!buffer)
                        ...
                        )
{
    URLContext *h;
    int err;
    /** 主要任务是 打开文件链接，并且填充一个 URLContext 结构体 */
    err = ffurl_open_whitelist(&h, filename, flags, int_cb, options, whitelist, blacklist, NULL);
    if (err < 0)
        return err;
    /** 根据URLContext 对象初始化 AVIOContext */
    err = ffio_fdopen(s, h);
    if (err < 0) {
        ffurl_close(h);
        return err;
