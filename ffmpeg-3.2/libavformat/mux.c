...
static int write_header_internal(AVFormatContext *s)
{
	...
	// 函数指针，指向写文件头的函数，不同的avoutputformat 有不同的write_header 实现，在具体的文件中实现
...
{	/**
	  *  1、计算avpacket 的duration 以及dts 等信息，另一方面检查pts 和 dts 这些参数的合理性
	  */
