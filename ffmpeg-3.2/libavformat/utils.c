...
static const AVCodec *find_probe_decoder(AVFormatContext *s, const AVStream *st, enum AVCodecID codec_id)
{
    ...
    /** 如果编码器已经存在就根据编码器类型返回对应的编解码器，否则就根据id进行查找 */
                      ...
                      AVDictionary **options)
{
    ...
    if (s->pb) {
        ...
        if (!s->iformat) {
            /** 
             *  如果已经制定了pb，但是没有指定 iformat，以 pb 读取媒体数据进行数据探测，以获得 iformat
             */
            return av_probe_input_buffer2(s->pb, &s->iformat, filename,
                                         s, 0, s->format_probesize);
        }
        else if (s->iformat->flags & AVFMT_NOFILE) {
            /** 如果已经制定pb, 也指定了 iformat，但是又指定不需要文件，这就会出现矛盾，
             *  这个时候不需要操作文件。此时下面作为一个提示输出
             */
    ...
    /*
     * 2、如果已经指定了 iformat 并且设置了不需要文件的标志，此时不需要pb 就可以直接返回；
     *    如果没有指定iformat， 但是可以从文件名中猜出 iformat , 也算是成功
     */
    ...
    /*
     * 如果从文件名中也猜不出媒体格式，则只能打开这个文件进行探测了，先打开文件，然后再调用 av_probe_input_buffer2 探测文件.
     * 此处主要是查找url_protocol 以及使用它初始化AVIOContext, 实现对 url_protocol 的一次封装抽象
     */
...
/**
 *  打开一个输入流并且读它的头部信息，编解码器不会被打开，这个方法调用结束后，一定要调用avformat_close_input()关闭流
 *  [注意]：
 *    1、如果fmt参数非空，也就是人为的指定了一个AVInputFormat的实例，那么这个函数就不会再检测输入文件的格式；
 *    2、如果fmt为空，那么这个函数就会自动探测输入文件的格式等信息；
 */
                        ...
                        AVInputFormat *fmt, AVDictionary **options)
{
    ...
    /** 调用init_input函数初始化输入流的信息。这里会初始化AVInputFormat */
    ...
    if (s->pb)/** 从 mp3 文件中红读取ID3数据，并保存它 */
        ff_id3v2_read(s, ID3v2_DEFAULT_MAGIC, &id3v2_extra_meta, 0);

    if (!(s->flags&AVFMT_FLAG_PRIV_OPT) && s->iformat->read_header)
        /** 读取文件头，
         *  主要做某种格式的初始化工作，比如填充自己的私有结构，根据流的数量分配流结构并且初始化，将文件指针指向数据区开始处等 
         */
...
int ff_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        /** 初始化外部 packet */
        av_init_packet(pkt);
        //关键：读取Packet , 调用对应的函数指针，读取 packet 数据
...
static int read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while (!got_packet && !s->internal->parse_queue) {
        ...
        /* read next packet, 从相应的AVInputFormat读取数据 */
...
static void update_stream_timings(AVFormatContext *ic)
{
    ...
    if (ic->pb && (filesize = avio_size(ic->pb)) > 0 && ic->duration > 0) {
        /* compute the bitrate */
        /** 
         * 计算码率: bitrate = (double) filesize * 8.0 * AV_TIME_BASE / (double) ic->duration;
         * 一字节=8bit, filesize*8是这个文件的所有bit数;
...
static void estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    /** 
     *  调用update_stream_timings函数更新流的时间相关的参数
     */
    update_stream_timings(ic);
                            ...
                            AVDictionary **options)
{
    ...
    /** has_codec_parameters 判断AVStream中的成员变量是否都已经设置完毕  */
...
/*            if (!st->time_base.num)
                st->time_base = *//** 设置 stream 的 timebase */
...
FF_ENABLE_DEPRECATION_WARNINGS
...
#endif
        /** fps_first_dts和fps_last_dts 用于帧率的计算 */
    ...
    for (;;) {
        ...
        /** 检查用户有没有请求中断。如果有中断请求，就调用中断回调方法，并且函数返回 */
         ...
         *  再一次遍历流，检查是不是还有编解码器需要进一步处理。这里会检查编解码器的各个参数，如果这些参数都就绪，
         *  那么就会返回了，也就没有必要再做进一步分析了。如果还有些编解码器的信息不全，那么这里会继续向下执行
         */
        for (i = 0; i < ic->nb_streams; i++) {
         ...
         * 从媒体文件中读取一帧，这儿是mp4格式就是读取一个sample
         */
         ...
         *  把读出来的数据添加到缓冲区。使用的是add_to_pktbuf函数。然后更新读到的总的数据的大小
         */
        ...
        /** 检查dts的连续性。 如果dts中的差异大于序列中平均分组持续时间的1000倍，我们将其视为不连续 */
    ...
    /** 
     *  处理音频流和视频流:
     *  1、对视频流而言，首先会获得原始数据的图像格式，然后据此找到对应的编解码器的tag。之后会计算平均帧率；
     *  2、对音频流而言，基于音频流的服务类型初始化disposition；
     */
    for (i = 0; i < ic->nb_streams; i++) {
        st = ic->streams[i];
        avctx = st->internal->avctx;
    ...
    /** 
     *  更新各个结构的数据
     *  AVStream中的AVCodecContext结构体，以及AVStream中AVStreamInternal中的AVCodecContext结构体的数据要统一起来。
     *  AVStream中的AVCodecParameters和AVStream中AVStreamInternal中的AVCodecContext结构体的数据也要统一起来。
     */
    for (i = 0; i < ic->nb_streams; i++) {
        st = ic->streams[i];
                        ...
                        AVCodec **decoder_ret, int flags)
{
    ...
    for (i = 0; i < nb_streams; i++) {
        ...
        count = st->codec_info_nb_frames;/** 在 av_find_stream_info 接口中已经解封装的帧数 */
        bitrate = par->bit_rate;
        multiframe = FFMIN(5, count);
        
        /**
         *  判断最合适的codec，选择解帧更多、码率更大、解码帧数更多的 codec
         */
