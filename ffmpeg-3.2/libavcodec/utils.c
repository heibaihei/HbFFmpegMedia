...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx, const AVCodec *codec, AVDictionary **options)
{
    ...
    /** 编码器上下文环境中定义了白名单和黑名单，不在白名单列表编码器就不行。这里检测到所给的编码器如果不在白名单中，这个函数直接退出。 */
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (avctx->codec->pix_fmts) {
            for (i = 0; avctx->codec->pix_fmts[i] != AV_PIX_FMT_NONE; i++) {
                /** 将AVCodecContext中设定的pix_fmt与编码器AVCodec中的pix_fmts数组中的元素逐一比较;
                 *  AVCodec中的pix_fmts数组存储了该种编码器支持的像素格式, AV_PIX_FMT_NONE 为结尾.
                 */
            ...
            {
                /**
                 * 如果发现AVCodec->pix_fmts数组中索引为i的元素是AV_PIX_FMT_NONE（即最后一个元素，取值为-1）的时候，就认为没有找到合适的像素格式，并且最终提示错误信息。
                 */
                char buf[128];
    ...
    if (av_codec_is_decoder(avctx->codec)) {
        /** 设置波特率 */
        if (!avctx->bit_rate) {
            /** 对于视频解码器，波特率就是AVCodecContext 下的bit_rate成员的值，音频解码器则会重新计算 */
...
{
    /** 判断编解码器有没有打开过就是判断它内部的internal字段是否为空, 空表示没有打开过，非空表示打开过了 */
    return !!s->internal;
}
