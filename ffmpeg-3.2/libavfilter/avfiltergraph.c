...
static int graph_check_validity(AVFilterGraph *graph, AVClass *log_ctx)
{
    ...
    /** 只有 graph 中 pads 都连接在一起，表明这个graph 才是有效的 */
    for (i = 0; i < graph->nb_filters; i++) {
        ...
        for (j = 0; j < filt->nb_outputs; j++) {
            if (!filt->outputs[j] || !filt->outputs[j]->dst) {/** 只要其中有一段 filter 为空，则该filter 就是无效的 */
...
{
    int ret;
    /** 检查 graph 是否有效，只有对应的输入输出 filter 数组非空，才表明该 graph 才是有效的 */
