...
//int VIDEO_PIX_FMT = AV_PIX_FMT_NV12;
...
int AUDIO_SAMPLE_FMT = AV_SAMPLE_FMT_S16P;
...
static uint8_t *sExtradata = NULL;
static uint8_t *sAudioExtradata = NULL;
static long sExtradataUsedLength = 0;

// FFmpeg Utilities

//static AVCodecContext *sAudioCodecContext = NULL;
...
void addVideoStream(AVFormatContext *dest){
    ...
    AVStream *st;
    AVCodec *codec;
    char str_orientation[128] = {0};

    AVDictionary *opts = NULL;

    int frame_rate = 30, ret = 0;
    ...
    // add PPS SPS info for codecContext.
    videoCC->extradata = sExtradata;
    videoCC->extradata_size = (int)sExtradataUsedLength;
...
//    avcodec_get_context_defaults3(c, codec);
    ...
    LOGI("%s, --------- width %d height %d", __func__, c->width, c->height);
...
//    c->gop_size = c->time_base.den * 10;
    c->keyint_min = 20;
    c->pix_fmt = VIDEO_PIX_FMT;
//    c->max_b_frames = 2;
    if (c->codec_id == AV_CODEC_ID_H264)
    {
        av_opt_set(c->priv_data, "level", "4.1", 0);
        av_opt_set(c->priv_data, "preset", "superfast", 0);
        av_opt_set(c->priv_data, "tune", "zerolatency", 0);
    }

    st->avg_frame_rate.num = c->time_base.den;
    ...
    sprintf(str_orientation, "%d", VIDEO_ORIENTATION);
    av_dict_set(&st->metadata, "rotate", str_orientation, 0);

//    av_dict_set(&opts, "profile", "baseline", 0);
//    if (c && ((ret = avcodec_open2(c, codec, &opts)) < 0))
//    {
//        LOGE("frame recorder w:%d,h:%d,name:%s",c->width,c->height,c->codec->name);
...
void addAudioStream(AVFormatContext *formatContext){
    ...
    codec = avcodec_find_encoder((enum AVCodecID) AUDIO_CODEC_ID);
    if (!codec) {
    ...
    codecContext->codec_id = (enum AVCodecID) AUDIO_CODEC_ID;
...
//    codecContext->strict_std_compliance = FF_COMPLIANCE_UNOFFICIAL; // for native aac support
    ...
    codecContext->sample_fmt  = (enum AVSampleFormat) AUDIO_SAMPLE_FMT;
//    codecContext->time_base.den = AUDIO_SAMPLE_RATE;
//    codecContext->time_base.num = 1;
    codecContext->sample_rate = AUDIO_SAMPLE_RATE;
    codecContext->channels    = AUDIO_CHANNELS;

    LOGI("addAudioStream sample_rate %d index %d channels %d", codecContext->sample_rate, st->index, codecContext->channels);
//    LOGI("add_audio_stream parameters: sample_fmt: %d bit_rate: %d sample_rate: %d", codec_audio_sample_fmt, bit_rate, audio_sample_rate);
    ...
    for (index = 0; index < 16; index++)
        if (codecContext->sample_rate == avpriv_mpeg4audio_sample_rates[index])
            break;
    if (index == 16) {
        av_log(codecContext, AV_LOG_ERROR, "Unsupported sample rate %d\n",
               codecContext->sample_rate);
        ret = AVERROR(ENOSYS);
        goto error;
    }
    if (codecContext->flags & CODEC_FLAG_GLOBAL_HEADER) {
        codecContext->extradata_size = 2;
        codecContext->extradata = av_mallocz(codecContext->extradata_size +
        ...
        codecContext->extradata[0] = 0x02 << 3 | index >> 1;
        codecContext->extradata[1] = (index & 0x01) << 7 | codecContext->channels << 3;
    }
...
AVFormatContext* avFormatContextForOutputPath(const char *path, const char *formatName){
    ...
    if (openOutputValue < 0) {
        const char *error = av_err2str(openOutputValue);
        LOGE("%s error. error info:%s",__func__, error);
        avformat_free_context(outputFormatContext);
    } else {
        outputFormat = outputFormatContext->oformat;
        if (outputFormat->video_codec != AV_CODEC_ID_H264) {
            LOGI("warning: change format from %s to %s.\n",
                 avcodec_get_name(outputFormat->video_codec),
                 avcodec_get_name(AV_CODEC_ID_H264));
            outputFormat->video_codec = AV_CODEC_ID_H264;
        }
        if (outputFormat->audio_codec != AV_CODEC_ID_AAC) {
            LOGI("warning: change format from %s to %s.\n",
                 avcodec_get_name(outputFormat->audio_codec), avcodec_get_name(AV_CODEC_ID_AAC));
...
Java_com_meitu_media_editor_FFmpegWrapper_setAVOptions(JNIEnv *env, jobject instance,
                                                       jobject jOpts) {
    LOGI("%s", __func__);
    ...
    jfieldID jOrientationId = (*env)->GetFieldID(env, ClassAVOptions, "orientation", "I");
    ...
    jfieldID jExtradataBufferObject = (*env)->GetFieldID(env, ClassAVOptions, "extradata", "Ljava/nio/ByteBuffer;");
    LOGI("----- jExtradataBufferObject %p", jExtradataBufferObject);
    jobject bytebuffer = (*env)->GetObjectField(env, jOpts, jExtradataBufferObject);
    LOGI("----- bytebuffer %p", bytebuffer);

    long length = (*env)->GetDirectBufferCapacity(env, bytebuffer);
    uint8_t *data = (*env)->GetDirectBufferAddress(env, bytebuffer);
//    int i = 0;
//    LOGI("extradata %ld", length);
//    for (i = 0; i < length; ++i) {
//        LOGI("--data %d", data[i]);
//    }
    long sExtradataLength = length + FF_INPUT_BUFFER_PADDING_SIZE;
    sExtradata = av_mallocz(sExtradataLength);
    memcpy(sExtradata, data, (size_t)length);
    sExtradataUsedLength = length;
    ...
    VIDEO_ORIENTATION = (*env)->GetIntField(env, jOpts, jOrientationId);
 ...
 */
JNIEXPORT void JNICALL
Java_com_meitu_media_editor_FFmpegWrapper_prepareAVFormatContext(JNIEnv *env, jobject instance,
                                                                 jstring jOutputPath) {
    init();
 ...
 */
JNIEXPORT void JNICALL
Java_com_meitu_media_editor_FFmpegWrapper_writeAVPacketFromEncodedData(JNIEnv *env,
                                                                       jobject instance,
                                                                       jobject jData, jint jIsVideo,
                                                                       jint jOffset, jint jSize,
                                                                       jint jFlags, jlong jPts, jboolean jIsKeyFrame) {
    AVPacket pkt = { 0 };
    av_init_packet(&pkt);
    ...
    pkt.pts = av_rescale_q(pkt.pts, *videoSourceTimeBase, (outputFormatContext->streams[pkt.stream_index]->time_base));
//    pkt.pts = new_pts;
...
//    LOGI("packet size %d, jPts %lld pts %lld", pkt.size, jPts, pkt.pts);

    int writeFrameResult = av_interleaved_write_frame(outputFormatContext, &pkt);
 ...
 */
JNIEXPORT void JNICALL
Java_com_meitu_media_editor_FFmpegWrapper_finalizeAVFormatContext(JNIEnv *env, jobject instance) {
