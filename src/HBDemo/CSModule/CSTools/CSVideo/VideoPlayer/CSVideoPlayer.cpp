...
int CSVideoPlayer::start() {
    return HB_OK;
}

int CSVideoPlayer::stop() {
    return HB_OK;
}
...
int CSVideoPlayer::setPlayList(CSPlayList *pPlayList) {
    if (mPlayList)
        SAFE_DELETE(mPlayList);
    mPlayList = pPlayList;
    mPlayList->setOpaque(this);
    LOGI("Video Player >>> Set new play list<%p> !", mPlayList);
    return HB_OK;
}
}
