...
struct _rbuf_s {
    u_char *buf;        /** 指向 buffer 缓冲区指针 */
    int size;           /** 标识 buffer 缓冲区的空间大小 */
    int available;      /** 标识 buffer 可用空闲缓冲区的大小 */
    int used;           /** 标识已经填充了有效数据的 buffer 缓冲区的空间大小 */
    int rfx;            /** 标识外部可以读取缓冲区中数据的起始位置 */
    int wfx;            /** 标识外部可以往缓冲区中写数据的起始位置 */
