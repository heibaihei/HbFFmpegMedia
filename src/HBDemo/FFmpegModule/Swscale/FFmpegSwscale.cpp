 ...
 *
 * ±æ≥Ã–Ú π”√libswscale∂‘œÒÀÿ ˝æ›Ω¯––Àı∑≈◊™ªªµ»¥¶¿Ì°£
 * À¸÷– µœ÷¡ÀYUV420P∏Ò Ω◊™ªªŒ™RGB24∏Ò Ω£¨
 * Õ¨ ±Ω´∑÷±Ê¬ ¥”480x272¿≠…ÏŒ™1280x720
 * À¸ «◊ÓºÚµ•µƒlibswscaleµƒΩÃ≥Ã°£
 *
...
{
	//Parameters	
	FILE *src_file =fopen(RESOURCE_SWSCALE_ROOT_PATH"/sintel_480x272_yuv420p.yuv", "rb");
    if (!src_file)
        return -1;
	...
	FILE *dst_file = fopen(RESOURCE_SWSCALE_ROOT_PATH"/output/sintel_1280x720_rgb24.rgb", "wb");
	...
	//Show AVOption
	av_opt_show2(img_convert_ctx,stderr,AV_OPT_FLAG_VIDEO_PARAM,0);
	//Set Value
	...
	*/
	while(1)
	{
        // 从文件中读取指定大小数据到 temp_buffer 缓冲区中
